...
export class FortyKActor extends Actor {
    ...
    static async create(data, options) {
        ...
        if (data.type === "npc") {
            foundry.utils.mergeObject(data, { "prototypeToken.appendNumber": true });
        ...
        if (actorData.type === "dwPC" || actorData.type === "npc" || actorData.type === "vehicle") {
            ...
            if (newFatigue) {
                ...
                if (newFatigue >= this.system.secChar.fatigue.max * 2) {
                    await game.fortyk.FortykRolls.applyDead(token, this, null, "fatigue");
                } else if (
                    !this.getFlag("core", "frenzy") &&
                    !this.getFlag("core", "unconscious") &&
                    newFatigue >= this.system.secChar.fatigue.max
                ) {
                    let effect = [];
                    effect.push(
            ...
            if (vehicle) {
                ...
                if (
                    (wounds && (this.system.horde.value || this.system.formation.value)) ||
    ...
    prepareBaseData() {
        let actorData = this;
        if (actorData.flags.fortyk === undefined) {
            actorData.flags.fortyk = {};
        }
    ...
    _prepareCharacterBaseData(data) {
        ...
        if (game.settings.get("fortyk", "bcCorruption")) {
            corruption.value =
                ...
                parseInt(corruption.khorne) +
                parseInt(corruption.tzeentch) +
                parseInt(corruption.slaanesh) +
                parseInt(corruption.nurgle) +
                parseInt(corruption.malice);
        }
        ...
        if (this.getFlag("fortyk", "sanguinethirst") && this.getFlag("core", "frenzy")) {
            ...
            if (butcher) {
                data.characteristics.wp.value -= butcher * 5;
            }
        }
        ...
        this.items.forEach((fortykItem, id, items) => {
            ...
            if (item.type === "skill") {
                let name = "";
                if (item.system.parent.value) {
                    name += item.system.parent.value.toLowerCase() + ":";
                }
        ...
        if (actorData.type === "dwPC") {
            ...
            this.items.forEach((fortykItem, id, items) => {
                ...
                if (item.type === "mission") {
                    ...
                    data.characteristics.inf.advance =
                        parseInt(data.characteristics.inf.advance) + parseInt(item.system.inf.value);
                }
                if (
                    item.type === "meleeWeapon" ||
                    item.type === "rangedWeapon" ||
                    item.type === "forceField" ||
                    item.type === "wargear" ||
                    item.type === "ammunition" ||
                    item.type === "consummable" ||
                    item.type === "armor" ||
                    item.type === "mod"
                ) {
                    //total weight calcs
                    ...
                    if (item.system.weight.total === "NaN") item.system.weight.total = "0.00";

                    data.carry.value = (parseFloat(data.carry.value) + parseFloat(item.system.weight.total)).toFixed(2);
                    if (!data.carry.value) data.carry.value = 0.0;
                }
                var twohand = false;
                if (item.type === "rangedWeapon") {
                    if (!this.getFlag("fortyk", "irongrip")) {
                        if (
                            (this.getFlag("fortyk", "firmgrip") && item.system.class.value !== "Heavy") ||
                            item.system.class.value === "Pistol" ||
                            item.system.class.value === "Thrown" ||
                            item.getFlag("fortyk", "pistolgrip")
                        ) {
                        } else {
                            twohand = true;
                        }
            ...
            this.items.forEach((fortykItem, id, items) => {
                ...
                if (
                    item.type === "psychicPower" &&
                    item.getFlag("fortyk", "sustained") &&
                    item.system.sustain.value !== "No"
                ) {
                    data.psykana.pr.sustained.push(item.id);
                }
            });
    ...
    prepareDerivedData() {
        ...
        } else if (actorData.type === "owRegiment") {
            this._prepareRegimentData(actorData);
        } else if (actorData.type === "knightHouse") {
            this._prepareHouseData(actorData);
        }
    }
    _prepareRegimentData(actorData) {
        const data = this.system;
        data.characteristics.inf.total =
            parseInt(data.characteristics.inf.value) +
            parseInt(data.characteristics.inf.not) +
            parseInt(data.characteristics.inf.time) +
            parseInt(data.characteristics.inf.frontActive) +
            parseInt(data.characteristics.inf.war) +
            parseInt(data.characteristics.inf.mod);
    }
    /**
    ...
    _prepareCharacterData(actorData) {
        ...
        if (data.characteristics.s.bonus + data.characteristics.t.bonus > 19) {
            ...
            data.carry.max =
                game.fortyk.FORTYK.carry[data.characteristics.s.bonus + data.characteristics.t.bonus].carry +
                data.carry.mod;
        } else {
    ...
    _prepareNPCData(actorData) {
        ...
        for (let [key, char] of Object.entries(data.characteristics)) {
            if (
                !this.getFlag("core", "frenzy") &&
                !this.getFlag("fortyk", "tireless") &&
                char.bonus * fatigueMult < data.secChar.fatigue.value
            ) {
    ...
    async _prepareVehicleData(actorData) {
        ...
        if (knight.chassis) {
            ...
            if (knight.armor) {
                ...
                data.knight.space.value += Math.ceil(
                    parseFloat(data.knight.armor.system.space.value) * parseFloat(data.knight.space.max)
                );
                data.knight.armorValues.max = Math.ceil(
                    data.knight.armorValues.max * parseFloat(data.knight.armor.system.armor.mod)
                );
                data.knight.armorValues.sideMax = Math.ceil(
                    data.knight.armorValues.sideMax * parseFloat(data.knight.armor.system.armor.mod)
                );
            }
            if (knight.structure) {
                data.knight.structure = this.getEmbeddedDocument("Item", knight.structure);
                data.knight.space.value += Math.ceil(
                    parseFloat(data.knight.structure.system.space.value) * parseInt(data.knight.space.max)
                );
                data.knight.tonnage.value += parseFloat(data.knight.structure.system.weight.value);
                data.secChar.wounds.max = Math.ceil(
                    parseInt(data.secChar.wounds.max) * parseFloat(data.knight.structure.system.SI.mod)
                );
                data.knight.space.max = Math.ceil(
                    parseInt(data.knight.space.max) * parseFloat(data.knight.structure.system.space.mod)
                );
            }
    ...
    prepareItems(data) {
        ...
        try {
            ...
            rightHandWeapon = undefined;
        }
        var leftHandWeapon;
        try {
            leftHandWeapon = data.secChar.wornGear.weapons[1];
        } catch (err) {
            leftHandWeapon = undefined;
        }
        ...
        if (this.type !== "npc") {
            ...
            for (let i = 0; i < skills.length; i++) {
                ...
                item.system.total.value +=
                    parseInt(item.system.value) +
                    parseInt(item.system.mod.value) +
                    parseInt(data.characteristics[item.system.characteristic.value].total);
    ...
    deleteItem(itemId) {
        ...
        if (item.type === "skill" && item.hasChildren) {
            let skills = this.items.filter(function (item) {
    ...
    weaponParry(weapon) {
        if (
            jQuery.isEmptyObject(weapon) ||
            (weapon.type === "rangedWeapon" && weapon.system.class.value !== "Pistol")
        ) {
            return -20;
        }
        let parry = 0;
            ...
            for (const skill of splitSkills) {
                ...
                if (instance) {
                    ...
                    if (skillData.system.parent) {
                        let parent = actorSkills.find(
                            (skill) => skill.name.toLowerCase() === skillData.system.parent.value.toLowerCase()
                        );
                        if (parent) {
    ...
    getRepairs() {
        ...
        if (this.getFlag("fortyk", "firedamage")) {
            repairs.push({
                label: "Seconds of Fire Damage",
                amount: this.getFlag("fortyk", "firedamage"),
                type: "firedamage"
            });
        }
        ...
        if (thresh1dmg > 0) {
            repairs.push({
                label: "First Threshold Damage",
                amount: thresh1dmg,
                type: "firstthresholddmg",
                disabled: disabled1
            });
        }
        //2nd threshold
        ...
        if (thresh2dmg > 0) {
            repairs.push({
                label: "Second Threshold Damage",
                amount: thresh2dmg,
                type: "secondthresholddmg",
                disabled: disabled2
            });
        }
        //3rd threshold
        ...
        if (thresh3dmg > 0) {
            repairs.push({
                label: "Third Threshold Damage",
                amount: thresh3dmg,
                type: "thirdthresholddmg",
                disabled: disabled3
            });
        }
        //4th threshold
        ...
        if (thresh4dmg > 0) {
            repairs.push({
                label: "Fourth Threshold Damage",
                amount: thresh4dmg,
                type: "fourththresholddmg",
                disabled: disabled4
            });
        }
        //critical damage
        ...
        let frontArmorDmg = { amount: 0, effectIds: [], label: "Front armor damage", type: "armordmg" };
        let leftSideArmorDmg = { amount: 0, effectIds: [], label: "Left side armor damage", type: "armordmg" };
        let rightSideArmorDmg = { amount: 0, effectIds: [], label: "Right side armor damage", type: "armordmg" };
        let rearArmorDmg = { amount: 0, effectIds: [], label: "Rear armor damage", type: "armordmg" };
        ...
        AEs.forEach(function (effect) {
            if (effect.getFlag("fortyk", "repair")) {
                if (effect.getFlag("fortyk", "repair") === "armordmg") {
                    ...
                    repairs.push({
                        label: effect.name,
                        type: effect.getFlag("fortyk", "repair"),
                        amount: Math.abs(parseInt(effect.changes[0].value)),
                        uuid: effect.uuid
                    });
                }
            }
        ...
        items.forEach(function (item) {
            ...
            if (item.type === "forceField") {
                if (item.system.state.value === "D") {
                    repairs.push({
                        label: `${item.name} Damaged`,
                        amount: 1,
                        type: "damagedionshield",
                        uuid: item.uuid
                    });
                }
                if (item.system.state.value === "X") {
                    repairs.push({
                        label: `${item.name} Destroyed`,
                        amount: 1,
                        type: "destroyedionshield",
                        uuid: item.uuid
                    });
                }
            } else if (
                ...
                if (item.system.state.value === "D") {
                    repairs.push({
                        ...
                        rarity: item.system.rarity.value,
                        uuid: item.uuid
                    });
                }
                if (item.system.state.value === "X") {
                ...
                if (item.system.state.value === "X") {
                    repairs.push({
                        label: `${item.name} Destroyed`,
                        amount: Math.max(1, parseInt(item.system.space.value)),
                        type: "install/removecomponent",
                        uuid: item.uuid
                    });
                }
            }
    ...
    getScope() {
        ...
        if (!this.system.isPrepared) {
            ...
            pr =
                parseInt(system.psykana.pr.value) +
                parseInt(system.psykana.pr.bonus) -
                Math.max(0, parseInt(system.psykana.pr.sustain) - 1);
        }
        pr = Math.max(0, pr);
        let lastTest = this.getFlag("fortyk", "lasttest");
        let dos = lastTest?.dos;
        return {
            dos: dos,
