...
export class FortyKKnightSheet extends FortyKBaseActorSheet {
    ...
    async _prepareContext(options) {
        ...
        if (system.knight.house) {
            ...
            data.rangedWeapons = house.itemTypes.rangedWeapon.filter(
                (weapon) => weapon.system.class.value === "Titanic Ranged Weapon"
            );
            data.artilleryWeapons = house.itemTypes.rangedWeapon.filter(
                (weapon) => weapon.system.class.value === "Titanic Artillery Weapon"
            );
            ...
            data.sensors = house.itemTypes.knightComponent.filter(
                (component) => component.system.type.value === "sensor"
            );
            data.coreMods = house.itemTypes.knightComponent.filter(
                (component) => component.system.type.value === "core-mod"
            );
            data.throneMods = house.itemTypes.knightComponent.filter(
                (component) => component.system.type.value === "throne-mod"
            );
            data.armActuators = house.itemTypes.knightComponent.filter(
                (component) => component.system.type.value === "arm-actuator"
            );
            data.legActuators = house.itemTypes.knightComponent.filter(
                (component) => component.system.type.value === "leg-actuator"
            );
            data.platings = house.itemTypes.knightComponent.filter(
                (component) => component.system.type.value === "plating"
            );
            data.gyros = house.itemTypes.knightComponent.filter((component) => component.system.type.value === "gyro");
            data.others = house.itemTypes.knightComponent.filter(
                (component) => component.system.type.value === "other"
            );
        }
                    ...
                    buttons: {
                        submit: {
                            ...
                            callback: async (html) => {
                                ...
                                update["system.secChar.wounds.value"] =
                                    createdChassis[0].system.structuralIntegrity.value;
                    ...
                    buttons: {
                        submit: {
                            ...
                            callback: async (html) => {
                                ...
                                let createdSpirit = await actor.createEmbeddedDocuments("Item", [
                                    foundry.utils.duplicate(spiritDoc)
                                ]);
    ...
    async _onMeldClick(event) {
        ...
        let meldContent;
        let knightAeData = { name: "Meld Bonuses", changes: [] };
        ...
        let pilotAeData = { name: "Meld Bonus", changes: [] };
                            ...
                            if (house) {
                                try {
                                    ...
                                    let newLoans = loans.filter(
                                        (loan) => loan.knightId !== actor.id && loan.itemId !== itemId
                                    );
    ...
    async _onHouseChange(event) {
        ...
        if (oldHouse) {
            ...
            oldArray = oldArray.filter((knight) => {
                return knight !== this.actor.id;
            });
    ...
    async _onWeaponReload(event) {
        ...
        if (house) {
            ...
            originalAmmo.update({
                "system.amount.taken": originalAmmo.system.amount.taken - 1,
                "system.amount.value": parseInt(originalAmmo.system.amount.value) - 1
            });
        }
    }
    ...
    async _onKnightOverheat(event) {
        ...
        if (coreIntegrity === 0) {
            overheatResult =
                "The knight’s core goes critical, the knight suffers a core meltdown at the end of your next turn.";
            ...
            overheatResult =
                "The knight shuts down and is considered helpless until restarted. This is a +0 Operate: Titanic Walker test which takes a full action.";
            overheatFlavor = "Emergency Shutdown";
        } else if (result >= 2) {
            overheatResult =
                "The knight takes 4d10 damage ignoring armor each turn until the core overload is cleared. To clear the Core Overload the bondsman must either pass a +0 tech-use test as a full action or shut down the knight.";
            overheatFlavor = "Core Overload";
        } else if (result === 1) {
            overheatResult =
                "Roll a +0 tech-use test as a full action, on a success your knight is stunned for 1d5 rounds, on a failure the knight suffers a core meltdown in 1d5 rounds(rolled by the GM). You may retry the tech-use test each round.";
