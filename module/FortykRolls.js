...
returns the roll message*/
    ...
    static async fortykTest(
        ...
        if (
            type === "rangedAttack" ||
            type === "meleeAttack" ||
            (type === "focuspower" && fortykWeapon.flags.fortyk.attack)
        ) {
            attack = true;
        }
        //prepare chat output
        ...
        if (isVehicle && fortykWeapon) {
            if (fortykWeapon.getFlag("fortyk", "taxing")) {
                let newHeat =
                    parseInt(actor.system.knight.heat.value) + parseInt(fortykWeapon.getFlag("fortyk", "taxing"));
        ...
        if (((testResult && testRoll < 96) || testRoll === 1) && !jam) {
            if (dosHouseRule) {
                ...
                testDos =
                    Math.abs(Math.floor(target / 10) - Math.floor(rollValue / 10)) + 1 + Math.ceil(charObj.uB / 2);
            }
            ...
            if (
                attack &&
                actor.getFlag("fortyk", "weaponinstinct") &&
                fortykWeapon &&
                weapon.system.type.value === "Exotic"
            ) {
                testDos += 1;
            }
        ...
        if (attack && templateOptions["success"]) {
            ...
            let evadepenalty = 0;
            if (
                actor.getFlag("fortyk", "psyboltattunement") &&
                fortykWeapon.type === "rangedWeapon" &&
                fortykWeapon.getFlag("fortyk", "force") &&
                attackType !== "semi" &&
                attackType !== "full"
            ) {
                let pr = actor.system.psykana.pr.effective;
                let inescPenalty = 5 * pr;
            ...
            if (
                actor.getFlag("fortyk", "inescapableattack") &&
                attackType !== "semi" &&
                attackType !== "full" &&
                attackType !== "swift" &&
            ...
            if (tarActor) {
                if (game.user.isGM || tarActor.isOwner) {
                    ...
                    let socketOp = {
                        type: "setFlag",
                        package: { token: tokenId, value: evadepenalty, scope: "fortyk", flag: "evadeMod" }
                    };
        ...
        if (actor.isOwner) {
            ...
            let socketOp = {
                type: "settestflag",
                package: { success: templateOptions.success, dos: testDos, actor: actor.uuid }
            };
            await game.socket.emit("system.fortyk", socketOp);
        }
        ...
        if (delayMsg) {
            ...
            let renderedPopupTemplate = await foundry.applications.handlebars.renderTemplate(
                popupTemplate,
                templateOptions
            );
        ...
        if (attack && templateOptions["success"]) {
            ...
            update["system.secChar.lastHit.value"] = hitlocation.value;
            update["system.secChar.lastHit.label"] = hitlocation.label;
            update["system.secChar.lastHit.dos"] = testDos;
            update["system.secChar.lastHit.hits"] = hits;
            update["system.secChar.lastHit.vehicleHitLocation"] = vehicleHitlocation;
            update["system.secChar.lastHit.vehicle"] = vehicle;
            update["system.secChar.lastHit.facing"] = facing;
            update["system.secChar.lastHit.type"] = type;
        ...
        if (attack && (type === "rangedAttack" || (type === "focuspower" && testResult))) {
            //blast
            let blast = false;
            ...
            if (
                (weapon.system.type === "Launcher" || weapon.system.type === "Grenade") &&
                blast &&
                !testResult &&
                jam
            ) {
                ...
                if (targets.size > 0) {
                    ...
                    for (let i = 0; i < rof; i++) {
                        ...
                        if (i >= rof - missedHits) {
                            ...
                            if (modifiedAngle > 359) {
                                modifiedAngle = modifiedAngle - 360;
                            }
        ...
        if (type === "focuspower") {
            if (!reroll && weapon.system.action.value.toLowerCase() === "reaction") {
                let aeData = {};
                aeData.id = "evasion";
                aeData.name = "Evasion";
            ...
            if (psykerType !== "navigator") {
                ...
                if (
                    actor.getFlag("fortyk", "ascendeddivination") &&
                    weapon.system.discipline.value.toLowerCase() === "divination"
                ) {
                    if (push && firstDigit === secondDigit) {
                        phenom = true;
                    }
                } else if (!push && (firstDigit === secondDigit || testRoll === 100)) {
                    phenom = true;
                ...
                if (phenom) {
                    ...
                    var ret = await this.psychicPhenomena(mod, actor);
                    perils = ret.perils;
                    var ork = ret.ork;
                    if (perils && actor.getFlag("fortyk", "warpmenace")) {
                        let chatMenace = {
                            author: game.user,
                            speaker: { actor, alias: name },
                            content: "Warp menace rerolls perils of the warp result one time.",
                            classes: ["fortyk"],
                            flavor: "Warp menace"
                        };
                        await ChatMessage.create(chatMenace, {});
                        ret = await this.psychicPhenomena(mod, actor);
                        perils = ret.perils;
                        ork = ret.ork;
                    }
                    if (perils) {
    ...
    static async perilsOfTheWarp(actor, ork = false) {
        ...
        if (actor.getFlag("fortyk", "soulbound")) {
            ...
            await soulboundRoll.toMessage(
                {
                    speaker: ChatMessage.getSpeaker({ author: game.users.current }),
        ...
        if (ork) {
            perilsMessage = FORTYKTABLES.eadBang[perilsResult];
        }
        let chatPhenom = {
    ...
    static async damageRoll(
        ...
        let ignoreSON =
            fortykWeapon.type === "psychicPower" ||
            fortykWeapon.getFlag("fortyk", "force") ||
        ...
        if (fortykWeapon.getFlag("fortyk", "tearing") || actor.getFlag("fortyk", "blessingofwrath")) {
        ...
        if (
            actor.getFlag("fortyk", "cleansewithfire") &&
            (fortykWeapon.getFlag("fortyk", "flame") || fortykWeapon.getFlag("fortyk", "purifyingflame"))
        ) {
            let wpb = actor.system.characteristics.wp.bonus;
            let dPos = form.indexOf("d");
            let afterD = dPos + 3;
        ...
        if (actor.getFlag("fortyk", "hammerblow") && lastHit.attackType === "allout") {
            ...
            weapon.system.pen.value =
                parseInt(weapon.system.pen.value) + Math.ceil(actor.system.characteristics.s.bonus / 2);
        }
        if (targets === null) {
        ...
        let butcher = 0;
        if (
            actor.getFlag("fortyk", "sanguinethirst") &&
            actor.getFlag("core", "frenzy") &&
            lastHit.type === "meleeAttack"
        ) {
                ...
                for (let tar of targets) {
                    ...
                    if (!tarActor.getFlag("core", "dead")) {
                        ...
                        if (h > 0 || fortykWeapon.getFlag("fortyk", "randomlocation")) {
                            if (!vehicle) {
                                if (fortykWeapon.getFlag("fortyk", "tarHead")) {
                                    curHit.value = FORTYKTABLES.hitLocations[1].value;
                                    curHit.label = FORTYKTABLES.hitLocations[1].label;
                                } else {
                                ...
                                if (fortykWeapon.getFlag("fortyk", "tarHead")) {
                                    curHit.label = FORTYKTABLES.vehicleHitLocations[81].label;
                                    curHit.value = FORTYKTABLES.vehicleHitLocations[81].value;
                                } else if (!data.hasTurret.value) {
                                    if (randomLocation._total >= 81) {
                                        ...
                                        curHit.value =
                                            game.fortyk.FORTYKTABLES.vehicleHitLocations[randomLocation._total].value;
                                        curHit.label =
                                            game.fortyk.FORTYKTABLES.vehicleHitLocations[randomLocation._total].label;
                                    }
                                } else {
                                    curHit.value =
                                        game.fortyk.FORTYKTABLES.vehicleHitLocations[randomLocation._total].value;
                        ...
                        if (actor.getFlag("fortyk", "exploitweakness") && lastHit.aim) {
                            let perBonus = actor.system.characteristics.per.bonus;
                            let weakBonus = Math.ceil(perBonus / 2);
                            tarRighteous -= weakBonus;
                        }
                        if (actor.getFlag("fortyk", "hammerofdaemons") && daemonic) {
                            tarRighteous--;
                        }
                        ...
                        for (let t = 0; t < terms.length; t++) {
                            if (terms[t] instanceof foundry.dice.terms.Die) {
                                for (let r of roll.terms[t].results) {
                                    ...
                                    if (r.rerolled) {
                                        if (r.count > r.result) {
                                            proven += r.count - r.result;
                                        } else {
                                            primitive += r.count - r.result;
                                        }
                                    }
                                }
                        ...
                        if (jQuery.isEmptyObject(armorSuit)) {
                            armorSuit = await new Item({ type: "armor", name: "standin" }, { temporary: true });
                        }
                        ...
                        let curBarrier;
                        if(barriers[tarNumbr] === false){
                            curBarrier=data.secChar.barrier.value;
                            barriers[tarNumbr]=curBarrier; 

                        }else{
                            curBarrier=barriers[tarNumbr];
                        }
                        let barrier=false;
                        if(curBarrier>0){
                        ...
                        if (
                            !barrier&&!vehicle &&
                            actor.getFlag("fortyk", "killerseye") &&
                            lastHit.attackType === "called" &&
                            lastHit.dos >= data.characteristics.agi.bonus
                        ) {
                            ...
                            await this.critEffects(
                                ...
                                killerCrit,
                                curHit.value,
                                weapon.system.damageType.value,
                                ignoreSON,
                                activeEffects,
                                "Killer's Eye "
                            );
                        ...
                        if (fortykWeapon.getFlag("fortyk", "mindeater")) {
                            ignoreSoak = true;
                        }
                        ...
                        if (
                            vehicle &&
                            weapon.type === "rangedWeapon" &&
                            weapon.getFlag("fortyk", "tankhunter") &&
                            !self
                        ) {
                            let bsb;
                            if (actor.type === "vehicle") {
                        ...
                        //smite the unholy
                        if (
                            actor.getFlag("fortyk", "smitetheunholy") &&
                            tarActor.getFlag("fortyk", "fear") &&
                            ...
                            if (!isNaN(fear)) {
                                pen += fear;
                                damageOptions.results.push(
                                    `<span>Smite the unholy increases damage and penetration by ${tarActor.getFlag("fortyk", "fear")} against the target.</span>`
                                );
                            }
                        }
                        //razor sharp weapons
                        ...
                        if (
                            fortykWeapon.getFlag("fortyk", "ignoreNaturalArmor") &&
                            tarActor.getFlag("fortyk", "naturalarmor")
                        ) {
                        ...
                        if (!vehicle) {
                            ...
                            if (
                                tarActor.getFlag("fortyk", "telekinedome") &&
                                !actor.getFlag("fortyk", "telekinedome")
                            ) {
                        ...
                        if (fortykWeapon.getFlag("fortyk", "scatter")) {
                            if (distance >= parseInt(weapon.system.range.value) / 2) {
                                damageOptions.results.push(`<div class="chat-target flexcol">`);
                                damageOptions.results.push(
                                    `<span> Armor is doubled against Scatter weapon at this distance!</span>`
                                );
                                soak += armor;
                                damageOptions.results.push(`</div>`);
                            }
                        ...
                        if (
                            tarActor.getFlag("fortyk", "reactiveplating") &&
                            (damageType === "explosive" || damageType === "impact")
                        ) {
                            soak += Math.ceil(armor * 0.2);
                            damageOptions.results.push(
                                `<span>Reactive Plating is resistant against this damage type.</span>`
                            );
                        }
                        //resistant armor
                        ...
                        //resilient armor
                        let armorRes = armorSuit.getFlag("fortyk", damageType + "Res");
                        if (armorRes) {
                            soak += parseInt(armorRes);
                            damageOptions.results.push(
                                `<span>Armor is resilient against this damage type and gains ${armorRes}.</span>`
                            );
                        }
                        //warp weapon vs holy armor
                        if (
                            fortykWeapon.getFlag("fortyk", "warp") &&
                            !armorSuit.getFlag("fortyk", "holy") &&
                            !vehicle
                        ) {
                            maxPen = armor;
                        ...
                        if (!vehicle && fortykWeapon.getFlag("fortyk", "wpSoak")) {
                            ...
                            soak += wps;
                            damageOptions.results.push(
                                `<span>This attack targets the target's willpower instead of toughness!</span>`
                            );
                        } else if (
                            !vehicle &&
                            fortykWeapon.getFlag("fortyk", "felling") &&
                            parseInt(tarActor.system.characteristics.t.uB)
                        ) {
                        ...
                        if (
                            ...
                            if (!isNaN(daemonic)) {
                                ...
                                damageOptions.results.push(
                                    `<span>The attack ignores ${daemonic} soak from the daemonic trait.</span>`
                                );
                            }
                        }
                        damageOptions.results.push(`</div>`);
                        ...
                        if (
                            ...
                            if (!isNaN(tarActor.getFlag("fortyk", "fear"))) {
                                ...
                                damageOptions.results.push(
                                    `<span>Smite the Unholy increases damage by ${parseInt(tarActor.getFlag("fortyk", "fear"))}</span>`
                                );
                            }
                        }
                        //volkite logic
                        ...
                        if (fortykWeapon.getFlag("fortyk", "accurate") && lastHit.aim) {
                            if (actor.getFlag("fortyk", "marksmanshonor") || distance > 2) {
                                ...
                                damageOptions.results.push(
                                    `<span>Accurate extra damage: ${accRoll.dice[0].values.join("+")}</span>`
                                );
                        ...
                        //vicious weapon
                        var vicious = fortykWeapon.getFlag("fortyk", "vicious");
                        if (actor.getFlag("fortyk", "blessingofvengeance") && daemonic) {
                            if (vicious) {
                                vicious = parseInt(vicious) + 2;
                            } else {
                                vicious = 2;
                            }
                        }
                        if (vicious && tens) {
                            let viscDice = parseInt(vicious);
                            let viscForm = viscDice + "d10";
                            let viscRoll = new Roll(viscForm, {});
                            await viscRoll.evaluate();
                            damageOptions.results.push(
                                `<span>Vicious extra damage: ${viscRoll.dice[0].values.join("+")}</span>`
                            );
                            damage += viscRoll._total;
                        ...
                        if (
                            !self &&
                            !fortykWeapon.getFlag("fortyk", "ignoreCover") &&
                            !fortykWeapon.getFlag("fortyk", "spray") &&
                            cover &&
                        ...
                        if (
                            tarActor.getFlag("fortyk", "swarm") &&
                            !(
                                fortykWeapon.getFlag("fortyk", "spray") ||
                                fortykWeapon.getFlag("fortyk", "blast") ||
                                fortykWeapon.getFlag("fortyk", "flame") ||
                                fortykWeapon.getFlag("fortyk", "purifyingflame") ||
                                fortykWeapon.getFlag("fortyk", "scatter")
                            )
                        ) {
                            damage = Math.ceil(damage / 2);
                            damageOptions.results.push(
                        ...
                        //gauss weapon logic
                        if (!barrier&&fortykWeapon.getFlag("fortyk", "gauss") && tens && !isHordelike) {
                            ...
                            let gaussActiveEffect = foundry.utils.duplicate(
                        ...
                        if (!barrier&&fortykWeapon.getFlag("fortyk", "corrosive") && !isHordelike) {
                            let corrosiveAmt = new Roll("1d10", {});
                            await corrosiveAmt.evaluate();
                            let id = foundry.utils.randomID(5);
                            damageOptions.results.push(
                                `<label class="popup" data-id="${id}"> Corrosive Weapon armor damage: ${corrosiveAmt._total}. <span class="popuptext chat-background" id="${id}">Excess corrosion is transferred to damage.</span></label> `
                            );
                            ...
                            let corrodeActiveEffect = foundry.utils.duplicate(
                                game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("corrode")]
                            );
                        ...
                        if (
                            !barrier&&!vehicle &&
                            damage > 0 &&
                            !isNaN(parseInt(toxic)) &&
                            !tarActor.getFlag("fortyk", "stuffofnightmares") &&
                            !tarActor.getFlag("fortyk", "undying") &&
                            !isHordelike
                        ) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let toxicMod = toxic * 10;
                            if (
                                tarActor.getFlag("fortyk", "resistance") &&
                                tarActor.getFlag("fortyk", "resistance").toLowerCase().includes("toxic")
                            ) {
                                toxicMod = -10;
                            }
                        ...
                        let messages = [];
                        //soulrazor
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "soulrazor") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                        ...
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "shocking") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let shock = await this.fortykTest(
                                "t",
                                "char",
                                tarActor.system.characteristics.t.total,
                                tarActor,
                                "Resist shocking",
                                null,
                                false,
                            ...
                            if (!shock.value) {
                                ...
                                let id = foundry.utils.randomID(5);
                                damageOptions.results.push(
                                    `<label class="popup" data-id="${id}"> Stunned for ${shock.dos} rounds. <span class="popuptext chat-background" id="${id}">${tar.name} is stunned for ${shock.dos} rounds and takes 1 fatigue!</span></label>`
                                );
                                let newfatigue = 1;
                        ...
                        //Thirsting
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "thirsting") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);

                            damageOptions.results.push(`Thirsting weapon saps vigor and applies 1 level of fatigue!`);
                            let newfatigue = 1;
                            this._addFatigue(tarActor, newfatigue);
                        ...
                        //Enfeebling
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "enfeebling") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                        ...
                        //Degeneration
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "degeneration") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                        ...
                        //Voidchill
                        if (!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "voidchill") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                        ...
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "abyssalDrain") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let drain = await this.fortykTest(
                                "t",
                                "char",
                                tarActor.system.characteristics.t.total - 20,
                                tarActor,
                                "Resist abyssal drain",
                                null,
                                false,
                                "",
                                true
                            ...
                            if (!drain.value) {
                                ...
                                activeEffects.push(drainActiveEffect);
                                damageOptions.results.push(
                                    `Drained for ${strDmg.result} strength damage and ${tDmg.result} toughness damage!`
                                );
                            }
                            damageOptions.results.push(`</div>`);
                        }
                        //Plague Carrier
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "plaguecarrier") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let drain = await this.fortykTest(
                                "t",
                                "char",
                                tarActor.system.characteristics.t.total - 30,
                                tarActor,
                                "Resist plague",
                                null,
                                false,
                            ...
                            if (!drain.value) {
                                ...
                                let plagueEffect = foundry.utils.duplicate(
                                    game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("rad")]
                                );
                                plagueEffect.name = "Plagued";
                                ...
                                damageOptions.results.push(
                                    `Plagued for ${tDmg.result} toughness damage and contagious for the next seven rounds!`
                                );
                            }
                            damageOptions.results.push(`</div>`);
                        }
                        ...
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "cryogenic") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let cryo = await this.fortykTest(
                                "t",
                                "char",
                                tarActor.system.characteristics.t.total - 40,
                                tarActor,
                                "Resist freezing",
                                null,
                                false,
                            ...
                            if (!cryo.value) {
                                ...
                                let id = foundry.utils.randomID(5);
                                damageOptions.results.push(
                                    `<label class="popup" data-id="${id}"> Freezing for ${cryoRoll._total} rounds. <span class="popuptext chat-background" id="${id}">${tar.name} is freezing for ${cryoRoll.result} rounds and will take 2d10 toughness damage per round, freezing if reaching 0 toughness!</span></label>`
                                );
                            }
                            damageOptions.results.push(`</div>`);
                        }
                        ...
                        if (!barrier&&
                            !vehicle &&
                            !isNaN(parseInt(fortykWeapon.getFlag("fortyk", "hallucinogenic"))) &&
                            !isHordelike
                           ) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            ...
                            let hallu = await this.fortykTest(
                                "t",
                                "char",
                                tarActor.system.characteristics.t.total - halluMod,
                                tarActor,
                                "Resist hallucinogenic",
                                null,
                                false,
                                "",
                                true
                            ...
                            if (!hallu.value) {
                                let halluActiveEffect = foundry.utils.duplicate(
                                    game.fortyk.FORTYK.StatusEffects[
                                        game.fortyk.FORTYK.StatusEffectsIndex.get("hallucinogenic")
                                    ]
                                );
                        ...
                        if (!barrier&&!vehicle && damage > 0 && fortykWeapon.getFlag("fortyk", "crippling") && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let crippleActiveEffect = foundry.utils.duplicate(
                        ...
                        if (!barrier&&fortykWeapon.getFlag("fortyk", "luminagen") && !isHordelike) {
                            ...
                            damageOptions.results.push(
                                `The target is affected by Luminagen and suffers a -10 penalty to all evasion tests for ${lumiDuration} rounds!`
                            );
                            damageOptions.results.push(`</div>`);
                        }
                        //NIDITUS WEAPON
                        if (!barrier&&!vehicle && fortykWeapon.getFlag("fortyk", "niditus") && damage > 0) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            ...
                            if (tarActor.getFlag("fortyk", "warpinstability")) {
                                let warpinst = await this.fortykTest(
                                    "wp",
                                    "char",
                                    tarActor.system.characteristics.wp.total - 10,
                                    tarActor,
                                    "Warp instability niditus",
                                    null,
                                    false,
                                    "",
                                    true
                        ...
                        //flame weapon
                        if (!barrier&&
                            !armorSuit.getFlag("fortyk", "flamerepellent") &&
                            fortykWeapon.getFlag("fortyk", "flame") &&
                            !isHordelike
                           ) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let fire;
                            if (vehicle) {
                                fire = await this.fortykTest(
                                    "agi",
                                    "char",
                                    tarActor.system.crew.ratingTotal + facing.armor,
                                    tarActor,
                                    "Resist fire",
                                    null,
                                    false,
                                ...
                                fire = await this.fortykTest(
                                    "agi",
                                    "char",
                                    tarActor.system.characteristics.agi.total,
                                    tarActor,
                                    "Resist fire",
                                    null,
                                    false,
                        ...
                        if (!barrier&&
                            ...
                            if (vehicle) {
                                ...
                                fire = await this.fortykTest(
                                    "wp",
                                    "char",
                                    tarActor.system.characteristics.wp.total,
                                    tarActor,
                                    "Resist fire",
                                    null,
                                    false,
                            ...
                            if (!fire.value) {
                                ...
                                fireActiveEffect.flags = {
                                    fortyk: { damageString: `1d10+${actor.system.psykana.pr.effective}` }
                                };
                        ...
                        if (!barrier&&
                            vehicle &&
                            !tarActor.getFlag("fortyk", "platinginsulation") &&
                            fortykWeapon.getFlag("fortyk", "thermal")
                           ) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                        ...
                        //snare weapon
                        if (!barrier&&!vehicle && !isNaN(parseInt(fortykWeapon.getFlag("fortyk", "snare"))) && !isHordelike) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let snareMod = fortykWeapon.getFlag("fortyk", "snare") * 10;
                            let snare = await this.fortykTest(
                                "agi",
                                "char",
                                tarActor.system.characteristics.agi.total - snareMod,
                                tarActor,
                                "Resist snare",
                                null,
                                false,
                            ...
                            if (!snare.value) {
                                let id = foundry.utils.randomID(5);
                                damageOptions.results.push(
                                    `<label class="popup" data-id="${id}"> Immobilised. <span class="popuptext chat-background" id="${id}">${tar.name} is immobilised. An Immobilised target can attempt no actions other than trying to escape the bonds. As a Full Action, he can make a (-${snareMod}) Strength or Agility test to break free.</span></label>`
                                );
                                let snareActiveEffect = foundry.utils.duplicate(
                                    game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("snare")]
                                );
                        ...
                        if (!vehicle && !isNaN(parseInt(fortykWeapon.getFlag("fortyk", "blinding"))) && !isHordelike) {
                            ...
                            let blind = await this.fortykTest(
                                "agi",
                                "char",
                                tarActor.system.characteristics.agi.total - blindMod,
                                tarActor,
                                "Resist blind",
                                null,
                                false,
                            ...
                            if (!blind.value) {
                                let id = foundry.utils.randomID(5);
                                damageOptions.results.push(
                                    `<label class="popup" data-id="${id}"> Blinded for ${blind.dos} rounds. <span class="popuptext chat-background" id="${id}">${tar.name} is blinded for ${blind.dos} rounds!</span></label>`
                                );
                                let blindActiveEffect = foundry.utils.duplicate(
                        ...
                        if (!barrier&&
                            !vehicle &&
                            actor.getFlag("fortyk", "disarmingattack") &&
                            curHit.value.includes("arm") &&
                            damage > 0 &&
                            lastHit.attackType === "called"
                           ) {
                            let dos = lastHit.dos;
                            ...
                            if (!disarm.value) {
                                damageOptions.results.push(`<label>Disarmed and must get their weapon.</label>`);
                                let disarmActiveEffect = foundry.utils.duplicate(
                                    game.fortyk.FORTYK.StatusEffects[
                                        game.fortyk.FORTYK.StatusEffectsIndex.get("target")
                                    ]
                                );
                        ...
                        if (!barrier&&
                            !vehicle &&
                            actor.getFlag("fortyk", "cripplingattack") &&
                            curHit.value.includes("leg") &&
                            damage > 0 &&
                            lastHit.attackType === "called"
                           ) {
                            let dos = lastHit.dos;
                        ...
                        if (!barrier&&
                            !vehicle &&
                            actor.getFlag("fortyk", "centermass") &&
                            curHit.value.includes("body") &&
                            damage > 0 &&
                            lastHit.attackType === "called"
                           ) {
                            let dos = lastHit.dos;
                        ...
                        if (!barrier&&
                            ...
                            let stun = await this.fortykTest(
                                "t",
                                "char",
                                tarActor.system.characteristics.t.total - stunMod,
                                tarActor,
                                "Resist stun",
                                null,
                                false,
                            ...
                            if (damage > tarActor.system.characteristics.s.bonus) {
                                let proneActiveEffect = foundry.utils.duplicate(
                                    game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("prone")]
                                );
                                activeEffects.push(proneActiveEffect);
                                damageOptions.results.push(`<span>Knocked down.</span>`);
                                let knockRoll = new Roll("1d5");
                                await knockRoll.evaluate();
                            ...
                            !barrier&&vehicle &&
                            tarActor.getFlag("fortyk", "walker") &&
                            fortykWeapon.getFlag("fortyk", "concussive") &&
                            damage > 0
                        ) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let difficulty =
                                ...
                                damage +
                                data.crew.rating -
                                10 * parseInt(fortykWeapon.getFlag("fortyk", "concussive"));
                            let knockdown = await this.fortykTest(
                                "",
                                "crew",
                                difficulty,
                                tarActor,
                        ...
                        if (!barrier&&
                            !vehicle &&
                            tempDmg > 0 &&
                            tempDmg > newWounds[tarNumbr] &&
                            weapon.getFlag("fortyk", "piercing")
                           ) {
                            let parsePierce = parseInt(weapon.getFlag("fortyk", "piercing"));
                            damage += parsePierce;
                        ...
                        if (!barrier&&
                            ...
                            tempDmg > 0 &&
                            tempDmg > newWounds[tarNumbr] &&
                            actor.getFlag("fortyk", "deathdealer") &&
                            weapon.type.toLowerCase().includes(actor.getFlag("fortyk", "deathdealer").toLowerCase())
                           ) {
                        ...
                        if (!barrier&&
                            ...
                            tempDmg > 0 &&
                            tempDmg > newWounds[tarNumbr] &&
                            actor.getFlag("fortyk", "peerlesskiller") &&
                            lastHit.attackType === "called"
                           ) {
                            damage += 4;
                            chatDamage += 4;
                        ...
                        if (!barrier&&armorSuit.getFlag("fortyk", "impenetrable")) {
                            damage = Math.ceil(damage / 2);
                            if (damage > 0) {
                                let impOptions = {
                                    author: user._id,
                                    speaker: { actor, alias: tar.name },
                        ...
                        //weakness trait
                        if (!barrier&&tarActor.getFlag("fortyk", "weakness")) {
                            if (tarActor.getFlag("fortyk", "weakness").toLowerCase().includes(damageType)) {
                                ...
                                if (damage > 0) {
                                    let weaknessOptions = {
                                        ...
                                        classes: ["fortyk"],
                                        flavor: `${tar.name} is weak to damage type!`
                                    };
                        ...
                        //resilience trait
                        if (!barrier&&tarActor.getFlag("fortyk", "resilience")) {
                            if (tarActor.getFlag("fortyk", "resilience").toLowerCase().includes(damageType)) {
                                ...
                                if (damage > 0) {
                                    let weaknessOptions = {
                                        ...
                                        classes: ["fortyk"],
                                        flavor: `${tar.name} is resilient to damage type!`
                                    };
                                    messages.push(weaknessOptions);
                                }
                            }
                        ...
                        if (!barrier&&
                            !vehicle &&
                            actor.getFlag("fortyk", "headhunter") &&
                            curHit.value === "head" &&
                            damage > 0 &&
                            lastHit.attackType === "called"
                           ) {
                        ...
                        if (!barrier&&
                            damage > 0 &&
                            fortykWeapon.type === "psychicPower" &&
                            actor.getFlag("fortyk", "navigator")
                           ) {
                            await FortyKItem.navigatorPsyTalents(actor, [tar]);
                        }
                        //lidless stare
                        if (!barrier&&damage > 0 && fortykWeapon.getFlag("fortyk", "lidlessstare")) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let duration = 1;
                            ...
                            damageOptions.results.push(stunString);
                            let stunActiveEffect = foundry.utils.duplicate(
                                ...
                                if (targetInt >= 20) {
                                    ...
                                    if (!death.value) {
                                        ...
                                        if (fate > 0) {
                                            ...
                                            damageOptions.results.push(`<span>${tarActor.getName()} dies!</span>`);
                        ...
                        if (!barrier&&
                            !vehicle &&
                            !data.suddenDeath.value &&
                            !isHordelike &&
                            damage > 0 &&
                            newWounds[tarNumbr] - damage < 0 &&
                            tarActor.getFlag("fortyk", "truegrit")
                           ) {
                        ...
                        if (!barrier&&
                            vehicle &&
                            damage > 0 &&
                            damage > newWounds[tarNumbr] &&
                            tarActor.getFlag("fortyk", "artificerhull")
                           ) {
                            let critDamage = 0;
                        ...
                        if (!barrier&&
                            ...
                            damage > 0 &&
                            damage > newWounds[tarNumbr] &&
                            tarActor.getFlag("fortyk", "reinforcedarmour")
                           ) {
                            let critDamage = 0;
                            if (newWounds[tarNumbr] <= 0) {
                                critDamage = damage;
                        ...
                        //mind eater
                        if (!barrier&&fortykWeapon.getFlag("fortyk", "mindeater")) {
                            damageOptions.results.push(`<div class="chat-target flexcol">`);
                            let mindDamage = Math.ceil(damage / 4);
                            let drainActiveEffect = foundry.utils.duplicate(
                                game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("weakened")]
                            );
                            drainActiveEffect.transfer = false;
                        ...
                        if (!barrier&&!vehicle && tarActor.getFlag("fortyk", "toxic") && distance <= 1 && damage > 0) {
                        ...
                        if (!vehicle) {
                            if (data.horde.value && damage > 0) {
                                ...
                                if (fortykWeapon.getFlag("fortyk", "blast")) {
                                    ...
                                    damageOptions.results.push(
                                        `<span>Blast adds ${fortykWeapon.getFlag("fortyk", "blast")} damage.</span>`
                                    );
                                }
                            ...
                            if (data.formation.value && damage > 0) {
                                ...
                                if (fortykWeapon.getFlag("fortyk", "blast")) {
                                    ...
                                    if (tens) {
                                        damageOptions.results.push(
                                            `<span>Blast adds ${fortykWeapon.getFlag("fortyk", "blast")} further damage on righteous fury.</span>`
                                        );
                                        damage += fortykWeapon.getFlag("fortyk", "blast");
                                        chatDamage += fortykWeapon.getFlag("fortyk", "blast");
                                    }
                        ...
                        if(barrier){
                            curBarrier=curBarrier-damage;
                            curBarrier=Math.max(0,curBarrier);
                            damageOptions.results.push(`<span>${damage} damage absorbed by ${actor.system.secChar.barrier.name}</span>`);
                            if(curBarrier===0){
                                damageOptions.results.push(`<span>${actor.system.secChar.barrier.name} collapsed!</span>`);
                            }
                            damage=0;
                            chatDamage=0;
                        }
                        ...
                        damageOptions.results.push(`</div>`);
                        let renderedDamageTemplate = await foundry.applications.handlebars.renderTemplate(
                            damageTemplate,
                            damageOptions
                        );
                        ...
                        let crit = await this._righteousFury(
                            actor,
                            label,
                            weapon,
                            curHit,
                        ...
                        if (
                            !vehicle &&
                            tens &&
                            deathwatch & actor.getFlag("fortyk", "xenosbane") &&
                            actor.system.secChar.wounds.value >= curWounds &&
                            !isHordelike
                        ) {
                            let banetest = await this.fortykTest(
                                "t",
                                "char",
                                tarActor.system.characteristics.t.total,
                                tarActor,
                                `Resist Xenos Bane instant death`,
                                null,
                                false,
                                "",
                                true
                        ...
                        if (game.user.isGM || tar.isOwner) {
                            if (tarNumbr <= newWounds.length - 1) {
                                ...
                                await tarActor.update({ "system.secChar.wounds.value": newWounds[tarNumbr],
                                                       "system.secChar.barrier.value": barriers[tarNumbr]});
                                let explosions = extraDamage[tarNumbr];

                                explosions.forEach(async function (params) {
                                    await FortykRolls.ammoExplosion(
                                        params.vehicle,
                                        params.component,
                                        params.facing,
                                        params.extraDamage
                            ...
                            let socketOp = {
                                ...
                                package: {
                                    token: tokenId,
                                    value: newWounds[tarNumbr],
                                    path: "system.secChar.wounds.value"
                                }
                            };
                            await game.socket.emit("system.fortyk", socketOp);
                        }
        ...
        if (selfToxic !== false && selfToxic !== undefined && !self) {
            ...
            if (
                ...
                actor.getFlag("fortyk", "resistance").toLowerCase().includes("toxic")
            ) {
                toxicMod = -10;
            }
            let toxicTest = await this.fortykTest(
    ...
    static async _righteousFury(
        ...
        if (!vehicle && tar !== null && (tar.actor.system.horde.value || tar.actor.system.formation.value)) {
            crit = false;
        }
        ...
        if (crit && damage > 0) {
            ...
            if (tar !== null) {
                if (tar.actor.getFlag("fortyk", "superheavy")) {
                    await this.superHeavyRightEffects(
                        tar,
                        res,
                        curHit,
                        ignoreSON,
                        activeEffects,
                        `<span class="chat-righteous">Righteous Fury </span>`,
                        vehicleOptions
                    );
                } else if (vehicle) {
                    await this.vehicleCrits(
    ...
    static async _critMsg(hitLoc, mesHitLoc, mesRes, mesDmgType, actor, source = "", threshold = false) {
        ...
        if (testStr !== null) {
            ...
            for (let i = 0; i < testStr.length; i++) {
                ...
                let test = await this.fortykTest(
                    testParam[0],
                    "char",
                    target,
                    actor,
    ...
    static async critEffects(attacker, token, num, hitLoc, type, ignoreSON, activeEffects = [], source = "") {
        if (game.user.isGM || token.isOwner) {
            ...
            let socketOp = {
                type: "critEffect",
                package: { token: tokenId, num: num, hitLoc: hitLoc, type: type, ignoreSON: ignoreSON }
            };
            await game.socket.emit("system.fortyk", socketOp);
        }
    }
    ...
    static async energyHeadCrits(attacker, actor, num, ignoreSON, activeEffects = [], source = "") {
        ...
        switch (num) {
            ...
            case 5:
                ...
                activeEffects.push(ae);
                ae = foundry.utils.duplicate(
                    game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("fel")]
                );
                ae.changes = [
                    {
                        key: `system.characteristics.fel.value`,
    ...
    static async energyBodyCrits(attacker, actor, num, ignoreSON, activeEffects = [], source = "") {
        ...
        switch (num) {
            ...
            case 8:
                ...
                activeEffects.push(ae);
                ae = foundry.utils.duplicate(
                    game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("s")]
                );
                ae.changes = [
                    {
                        key: `system.characteristics.s.value`,
                ...
                activeEffects.push(ae);
                ae = foundry.utils.duplicate(
                    game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("agi")]
                );
                ae.changes = [
                    {
                        key: `system.characteristics.agi.value`,
    ...
    static async energyLegCrits(attacker, actor, num, leg, ignoreSON, activeEffects = [], source = "") {
        ...
        switch (num) {
            ...
            case 8:
                ...
                tTest = rolls.tests[0];
                ae = foundry.utils.duplicate(
                    game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("bleeding")]
                );
                activeEffects.push(ae);
                if (!tTest.value) {
                    ae = foundry.utils.duplicate(
    ...
    static async explosiveBodyCrits(attacker, actor, num, ignoreSON, activeEffects = [], source = "") {
        ...
        switch (num) {
            ...
            case 7:
                ...
                if (!tTest.value) {
                    ae = foundry.utils.duplicate(
                        game.fortyk.FORTYK.StatusEffects[game.fortyk.FORTYK.StatusEffectsIndex.get("unconscious")]
                    );
                    activeEffects.push(ae);
                }
                break;
            ...
            switch (hitLoc.value) {
                case "hull":
                    ...
                    components = components.concat(
                        ...
                        actor.itemTypes.forceField,
                        actor.itemTypes.knightComponent,
                        actor.itemTypes.knightCore
                    );
                    components = components.filter(
                        (component) => component.system.state !== "X" && component.system.state !== 0
                    );
    ...
    static async motiveCrits(token, num, hitLoc, ignoreSON, activeEffects, source, vehicleOptions) {
        ...
        switch (num) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (vehicle.system.secChar.speed.motive === "D") {
                    return;
                }
                ae = {};
    ...
    static async thresholdHullCrits(crossed, tar, activeEffects, source, vehicleOptions) {
        ...
        switch (crossed) {
            ...
            case 2:
                components = [];
                components = components.concat(
                    vehicle.itemTypes.ammunition,
                    vehicle.itemTypes.forceField,
                    vehicle.itemTypes.knightComponent,
                    vehicle.itemTypes.knightCore
                );
                components = components.filter(
                    (component) => component.system.state !== "X" && component.system.state !== 0
                );
                ...
                if (component) {
                    ...
                    if (component.type === "ammunition") {
                        compUpdate["system.state.value"] = "X";
                        rightMes = `${component.name} explodes dealing weapon damage!`;
                        vehicleOptions.explosions.push({
                            vehicle: vehicle,
                            component: component,
                            facing: facing,
                            extraDmg: ""
    ...
    static filterAeDupplicates(effects) {
        let filteredEffects = [];
        for (let i = 0; i < effects.length; i++) {
            ...
            for (const filteredEffect of filteredEffects) {
                if (effect.statuses && effect?.statuses?.includes(filteredEffect?.statuses[0])) {
                    ...
                    if (effect.changes) {
                        if (filteredEffect.changes) {
                            filteredEffect.changes = filteredEffect.changes.concat(effect.changes);
                        } else {
        ...
        if (effect.length > 0) {
            if (game.user.isGM || token.isOwner) {
                ...
                for (const newAe of effect) {
                    ...
                    for (let ae of actor.effects) {
                        if (
                            !ae.statuses.has("weakened") &&
                            !ae.statuses.has("buff") &&
                            newAe.statuses &&
                            ae.statuses.has(newAe.statuses[0])
                        ) {
                            skip = true;
                                        ...
                                        if((change.key===newChange.key)&&change.type===newChange.mode){
                                            ...
                                            if(change.type===CONST.ACTIVE_EFFECT_CHANGE_TYPES.override||change.type===CONST.ACTIVE_EFFECT_CHANGE_TYPES.custom){
                                                ...
                                                if(!isNaN(parseInt(newChange.value))){
                                                    ...
                                                    newChange.value=parseInt(newChange.value)+parseInt(change.value);
                                                }else{
                    ...
                    if (newAe.id === "prone" && actor.getFlag("fortyk", "graceoftheasuryani")) {
                        skip = (
                            await this.fortykTest(
                                "agi",
                                "char",
                                actor.system.characteristics.agi.total,
                                actor,
                                "Grace of the Asuryani"
                            )
                        ).value;
                    }
                    ...
                    if (
                        !ignoreSON &&
                        (newAe.id === "stunned" || newAe.id === "bleeding") &&
                        actor.getFlag("fortyk", "stuffoffnightmares")
                    ) {
                        skip = true;
                        this._sON(actor);
                    }
    ...
    static async ammoExplosion(vehicle, ammo, facing, extraDmg = "") {
        ...
        if (extraDmg) {
            explosionData.system.damageFormula.value += `+${extraDmg}`;
        }
    ...
    static async slaughterHealing(actor) {
        let slaughterForm = "1d10";
        let slaughterRoll = new Roll(slaughterForm, {});
        await slaughterRoll.evaluate({ async: false });
        slaughterRoll.toMessage({ flavor: "Rolling fueled from slaughter healing" });
        let healing = slaughterRoll._total;
        let actorWounds = actor.system.secChar.wounds.value;
        actorWounds = Math.min(actorWounds + healing, actor.system.secChar.wounds.max);
        if (game.user.isGM || actor.isOwner) {
            await actor.update({ "system.secChar.wounds.value": actorWounds });
        } else {
            let tokenId = actor.id;

            let socketOp = {
                type: "updateValue",
                package: { token: tokenId, value: actorWounds, path: "system.secChar.wounds.value" }
            };
            await game.socket.emit("system.fortyk", socketOp);
        }
    }
    static async applyDead(target, actor, attacker, cause = "") {
        ...
        if (attacker) {
            ...
            if (
                !isHordelike &&
                attacker.getFlag("fortyk", "sanguinethirst") &&
                attacker.getFlag("core", "frenzy") &&
                lastHit === "meleeAttack"
            ) {
    ...
    static async _addFatigue(actor, newfatigue) {
        ...
        if (game.user.isGM || actor.isOwner) {
            ...
            let socketOp = {
                type: "updateValue",
                package: { token: tokenId, value: newfatigue, path: "system.secChar.fatigue.value" }
            };
            await game.socket.emit("system.fortyk", socketOp);
        }
    }
    ...
    static async knockback(knockback, attackerToken, actorToken, randomAngle = false) {
        try {
            if (actorToken.actor.getFlag("fortyk", "standfirm")) {
                let chatFirm = {
                    author: game.user,
                    speaker: { actor, alias: actorToken.name },
                    content: "Stand firm prevents Knockback",
                    classes: ["fortyk"],
                    flavor: "Stand firm"
                };
                await ChatMessage.create(chatFirm, {});
